define(function(require) {
    ...
    This is the application "controller" module.
     ...
     The app "controlller" module holds the application logic in an true MVC architecture.
...
//            }.override(),
            ...
            beforeStart: function($super) {
                ...
                TelematicsOcids.forEach(function(ocid) {
                    var method = (ocid.type === 'Boolean' ? 'is' : 'get') + ocid.name;
                    if (necessaryProperties.indexOf(ocid.name) >= 0) {
                        view.setTmxData(ocid.name, null);
                        telematics[method]().done(function(resp) {
                            view.setTmxData(ocid.name, resp);
                        });
                        telematics.subscribe('Telematics.' + ocid.name, function(resp) {
                            // Log.log('CHANGED: ', ocid.name, resp);
                            view.setTmxData(ocid.name, resp);
                        }.bind(this));
                    }
                }.bind(this));
